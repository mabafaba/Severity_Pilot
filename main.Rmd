...
comparisons_df<-comparisons_df %>%
  ...
  mutate("difference to other locations" = other.numbers - numbers) %>% 
...
comparisons_df<-comparisons_df %>% rename(`Location` = independent.var.value,`Obverved in Location` = numbers, "Other locations" = other.numbers, "p Value (Bonferroni corrected)" = p.value.corrected)
...
colnames(sumstat)<-c("Location","p Value",paste("% difference with score",1:4))
sumstat
}
...
comparisons_df$Location<-questionnaire$question_get_choice_labels(comparisons_df$Location,default_disaggregation)
...
design$variables[,vars_for_factor_analysis] %>% lapply(table)
