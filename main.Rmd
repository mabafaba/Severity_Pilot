...
sum_stats<-data.frame(camp_name= sum_stats_table$independent.var.value, mean=round(sum_stats_table$numbers,2))

sum_stats$camp_name<-str_standardize_cxb_camps(sum_stats$camp_name)
cmp$camp_name<-str_standardize_cxb_camps(cmp$New_Camp_N)


cmp_data<-sp::merge(cmp, sum_stats, by= "camp_name", all.x=TRUE, all.y=FALSE)
...
cmp_data@data<-cmp_data@data %>% 
  mutate(camp_setting= ifelse(is.na(mean), "Not Assessed",
                              ifelse( camp_name %in% kbc, "Kutapalong Megacamp",
                                      ifelse(camp_name %in% iso,"Detached Camps","Southern Teknaf"))))


# cmp_data@data<-cmp_data@data %>% new_recoding(region,source = camp_name) %>% 
#   recode_to("Kutapalong",where.selected.any = kbc, otherwise.to = 'Not Assessed',na.to = 'Not Assessed') %>% 
#   recode_to("Detached Camps",where.selected.any = iso) %>% 
#   recode_to("Southern Teknaf",where.selected.any = st) %>% 
#   end_recoding()
# table(cmp_data@data$region,cmp_data@data$camp_name)

# cmp_data$region<-ifelse(cmp_data$camp_name %in%  kbc, "Kutapalong",
#        ifelse(cmp_data$camp_name%in% iso, "Isolated Camps","Southern Teknaf"))
...
ggplot(sfdf)+geom_sf(aes(fill=mean))+facet_wrap(vars(camp_setting))

limits<-c(min(sfdf$mean,na.rm = T),max(sfdf$mean,na.rm = T))
library(ggrepel)
sfdf %>% split(sfdf$camp_setting) %>% purrr::map(function(x){
  ggplot(x)+
    geom_sf(aes(fill=mean),color = 'white')+
    theme_minimal()+
    scale_fill_continuous(limits=limits, name= "Mean Severity Score")+
    theme(axis.text.x = element_blank(),axis.text.y = element_blank(),axis.ticks = element_blank())+geom_text_repel(aes(x = lon, y = lat, label = camp_name, )) 
})
