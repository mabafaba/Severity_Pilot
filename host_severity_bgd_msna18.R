...
Individual_to_HH<-df %>% 
  ...
  mutate(
    ...
    si.edu_formal_learn_space = ifelse(num_not_in_formal_school>=3,2, ifelse(num_not_in_formal_school>=1,1,0)),
    si.edu_non_formal_learn_space=ifelse(num_not_in_nonformal_school>=3,2, ifelse(num_not_in_nonformal_school>=1,1,0)),
    
    #IF ANY ADULT WORKED THEY GET S0, IF  ONLY CHILD WORKED S1, IF NO ONE WORKED S2 
    si.fsl_individual_worked= ifelse(num_working_adult>0,0,
...
more_individual_to_HH<-df %>% 
  ...
  summarise(
    ...
    si.health_mosq=ifelse(is.na(health_mosq_net),0,health_mosq_net),
    si.health_indiv_illness=ifelse(sum(individual_illness=="yes",na.rm=TRUE)>0,1,
...
hh_level<-hhy %>% 
  ...
  mutate(
    si.fsl_fcs= ifelse(FCS>42,0, ifelse(FCS>28 & FCS<=42,1,ifelse(FCS<=28,2,NA))),
    si.fsl_unsustainable_food= ifelse(main_source_food %in% unsustainable_food_sources,2,0),
    ...
    si.fsl_main_income=ifelse(sum_sust_income==0 & sum_unsut_income>0,2,ifelse(sum_sust_income>0 &sum_unsut_income>0,1,
                                                                               ifelse(sum_sust_income>0& sum_unsut_income==0,0,NA))),
    food_exp_ratio=spend_food/TotalExpenditure,
    si.fsl_food_exp= ifelse(food_exp_ratio>0.74, 2, ifelse(food_exp_ratio>0.5,1,ifelse(food_exp_ratio<=0.5,0,NA))),
    si.protection_feel_safe= ifelse(hh_safe=="yes",0,
    ...
    si.nfi_exp=ifelse(nfi_to_total>=0.25,2,ifelse(nfi_to_total>=0.15,1,0)),
    shelter_expenditure= spend_fix_shelter + spend_rent,
    shelter_to_total=shelter_expenditure/TotalExpenditure,
    si.shelt_exp=ifelse(shelter_to_total>=0.2, 2, ifelse(shelter_to_total<0.2,1,0)),
    si.shelt_topo=ifelse(ShelterType=="Jhuprie",2,
                         ifelse(ShelterType=="Kutcha",1,
                                ifelse(ShelterType %in% c("Pucca", "SemiPucca"),1,NA))),
    si.nfi_essential= ifelse(rowSums(.[essential_nfis])>1,2,
                             ifelse(rowSums(.[essential_nfis])==1,1,
                                    ifelse(rowSums(.[essential_nfis])<1,0,NA))),
    si.wash_improved_water_access = ifelse(is.na(improved_water_access),2,
    ...
    si.wash_defecation= ifelse(hh_defecation== "open_defecation",2,ifelse( hh_defecation=="communal_latrine",1,0)),
    si.wash_enough_water= ifelse(enough_water=="yes",0,2),
    si.wash_soap= ifelse(soap_handwash %in% c("yes_saw_soap", "yes_didnt_see_soap"),0,2),
